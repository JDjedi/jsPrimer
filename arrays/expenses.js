...
const expenseToAdd = function(accountToAdd, expenseDescription, amountToAdd) {
  ...
  accountToAdd.ledger.push({description: expenseDescription, amount: amountToAdd})
}

const incomeToAdd = function(accountToAdd, incomeDescription, amountToAdd) {
  accountToAdd.income.push(amountToAdd)
  accountToAdd.ledger.push({description: incomeDescription, amount: amountToAdd})
}
...
const getAccountSummary = function(accountToDisplay) {
    return `${accountToDisplay.name} has an account balance of $${accountToDisplay.balance}.`
}
